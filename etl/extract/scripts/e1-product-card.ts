...
async function scrapeCollections(urls: string[]) {
	...
	for (const url of urls) {
		...
		try {
				...
				({ base, currentCategory }) => {
					...
					return Array.from(productCards)
						.map((card) => {
							...
							const shopifyId = idInput
								? (idInput as HTMLInputElement).value
								: card.id.replace("product-item-", "");
							...
							const currentPrice =
								(parseInt(priceText.replace(/[^0-9]/g, ""), 10) || 0) * 100;

							const cutPriceEl = card.querySelector(".product-price--compare");
							const cutPriceText = cutPriceEl
								? cutPriceEl.textContent || "0"
								: "0";
							const compareAtPrice =
								(parseInt(cutPriceText.replace(/[^0-9]/g, ""), 10) ||
									currentPrice / 100) * 100;

							const soldOutBadge = card.querySelector(".badge--sold-out");
							const inStock = !soldOutBadge;

							const imgNodes = card.querySelectorAll(
							...
							const images = Array.from(imgNodes)
								...
								.filter(
									(imgUrl) =>
										imgUrl.includes("cdn.shopify.com") ||
										imgUrl.includes("kyari.co/cdn"),
								)
			...
			console.log(
				`✔️ Grabbed ${pagePlants.length} plants, tagged as [${categorySlug}].`,
			);
		} catch (error) {
			console.error(
				`❌ Failed to scrape ${url}. Skipping to next. Error:`,
				error,
			);
		}
	}
	...
	fs.writeFileSync(
		"./scraper/collection_plants.json",
		JSON.stringify(finalSmartPlants, null, 2),
	);
	console.log(
...
scrapeCollections(collectionUrls).catch((err) =>
