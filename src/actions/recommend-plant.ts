...
export async function getPlantRecommendations(preferences: {
	...
	try {
		...
		const scoredPlants = allPlants.map((plant) => {
			...
			const searchString =
				`${plant.description || ""} ${JSON.stringify(plant.specifications || {})}`.toLowerCase();

			// --- SUNLIGHT LOGIC ---
			if (preferences.sunlight === "low") {
				if (
					searchString.includes("low light") ||
					searchString.includes("indirect") ||
					searchString.includes("shadow")
				) {
					score += 30;
				}
				if (
					searchString.includes("direct sun") ||
					searchString.includes("bright light")
				) {
					score -= 20; // Penalty for recommending a sun-lover to a dark room
				}
			} else if (preferences.sunlight === "bright") {
				if (
					searchString.includes("bright") ||
					searchString.includes("direct") ||
					searchString.includes("sunlight")
				) {
					score += 30;
				}
			}
			...
			if (preferences.wateringHabit === "forgetful") {
				...
				if (
					searchString.includes("1-2 weeks") ||
					searchString.includes("2-3 weeks") ||
					searchString.includes("dry out")
				) {
					score += 30;
				}
				...
				if (
					searchString.includes("moist") ||
					searchString.includes("humidity") ||
					searchString.includes("frequent")
				) {
					score += 30;
				}
			}
		...
		const sortedPlants = scoredPlants.sort(
		...
		return {
			success: false,
			error: "Failed to search the database. Please try again later.",
		};
	}
}
