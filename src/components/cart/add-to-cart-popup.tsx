...
export function CartNotification() {
    const { items, lastAddedItem, showNotification, closeNotification } = useCartStore();
    ...
    return (
        <>
            ...
            <div className="fixed z-[100] w-full bg-white shadow-2xl transition-all duration-300 ease-out 
            ...
            >
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onClick={closeNotification} className="p-1 text-gray-400 hover:text-gray-600 transition-colors">
                        <X className="h-5 w-5" />
                    </button>
                </div>
                ...
                <div className="flex gap-4 mb-6">
                    <div className="relative h-24 w-24 bg-gray-100 rounded-lg overflow-hidden shrink-0">
                        {lastAddedItem.image ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-gray-400 text-xs font-bold uppercase">No Img</div>
                        )}
                    </div>
                    <div className="flex flex-col text-sm">
                        <span className="font-bold text-gray-900 leading-tight mb-1">{lastAddedItem.name}</span>
                        <span className="text-gray-500 mb-0.5">Size: Standard</span> 
                        <span className="font-medium text-gray-900 mb-1">{formatPrice(lastAddedItem.price)}</span>
