...
export function AddToCartButton(productInfo: ReceivedItemInfo) {
	...
	return (
		<Button onClick={() => addProductToCart(productInfo)}>Add To Cart</Button>
	);
}
...
export function CartQuantityControl({
	...
	const itemCount = useCart(
		(state) => state.items.find((item) => item.id === productId)?.itemCount,
	);

	return (
		<div
			className={cn(
				`flex items-center justify-start gap-2 mt-auto`,
				`${className}`,
			)}
		>
			<button
			...
			<span className="p-1 w-6 h-full text-sm text-center text-foreground font-semibold muted-background rounded-md ">
				{itemCount || 0}
			</span>
