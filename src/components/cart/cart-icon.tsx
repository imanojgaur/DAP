...
export const DoddleStickerCart = ({ className }: { className?: string }) => {
	return (
		<svg
			...
			focusable="false"
			className={cn(
				"w-12 h-12 cursor-pointer hover:-translate-y-1 hover:text-teal-600 transition-all duration-300",
				className,
			)}
			role="img"
...
export const BoutiqueCart = ({ className }: { className?: string }) => (
	<svg
		...
		strokeLinejoin="round"
		className={cn(
			"w-6 h-6 hover:text-[#C15B3D] hover:-translate-y-0.5 cursor-pointer transition-all duration-300",
			className,
		)}
		role="img"
