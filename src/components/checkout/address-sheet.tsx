                ...
                <form onSubmit={onSubmit} className="flex-1 flex flex-col">
                    <div className="p-6 space-y-6 flex-1">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Input name="pincode" defaultValue={address?.zipCode} required className="h-12 border-gray-300" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Label className="text-xs text-gray-500">City</Label>
                                    <Input name="city" defaultValue={address?.city} required className="h-12 border-gray-300" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-xs text-gray-500">State</Label>
                                    <Input name="state" defaultValue={address?.state} required className="h-12 border-gray-300" />
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-xs text-gray-500">House/ Flat/ Office No.</Label>
                                <Input name="house" defaultValue={houseValue} required className="h-12 border-gray-300" />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-xs text-gray-500">Road Name/ Area /Colony</Label>
                                <Input name="road" defaultValue={roadValue} required className="h-12 border-gray-300" />
                            </div>
                        </div>

                        <div className="flex items-center justify-between py-4 border-y border-gray-100 my-6">
                            <Label htmlFor="isDefault" className="text-sm font-medium text-gray-900 cursor-pointer">Use as default address</Label>
                    ...
                    <div className="p-6 border-t border-gray-100 bg-white">
                        <Button disabled={isLoading} type="submit" className="w-full h-14 rounded-full bg-[#111] text-white hover:bg-black font-medium text-lg">
                            {isLoading ? "Saving..." : address ? "Update Address" : "Ship to this Address"}
                        </Button>
                    </div>
                </form>
            </SheetContent>
