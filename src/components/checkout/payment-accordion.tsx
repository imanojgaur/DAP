...
export function PaymentAccordion({ addressId }: { addressId: string }) {
	...
	return (
		<Accordion type="single" collapsible className="w-full space-y-4">
			...
			<AccordionItem
				value="upi"
				className="border border-gray-200 rounded-xl px-2 data-[state=open]:bg-gray-50/50 transition-colors"
			>
			...
			<AccordionItem
				value="cod"
				className="border border-gray-200 rounded-xl px-2 data-[state=open]:bg-green-50/30 transition-colors"
			>
				<AccordionTrigger className="hover:no-underline px-4 py-5">
					<div className="flex items-start gap-4 text-left">
				...
				<AccordionContent className="px-4 pb-6 pt-2">
					<div className="p-4 bg-white border border-green-100 rounded-xl mb-4">
						<p className="text-sm text-gray-600 leading-relaxed">
							Order now and pay when your plants arrive. Our delivery partner
							will accept Cash or UPI scanning at the time of delivery.
						</p>
					</div>
					<Button
						disabled={isLoading}
					...
					>
						{isLoading
							? "Placing Order..."
							: `Confirm Order (₹${(total / 100).toLocaleString()})`}
					</Button>
				</AccordionContent>
			</AccordionItem>
		</Accordion>
