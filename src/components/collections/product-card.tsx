...
export function ProductCard({ plant }: { plant: PlantCardData }) {
	...
	const discountPercent = calculateDiscountPercentage(plant.price, plant.compareAtPrice);
	const currentPrice = formatPrice(plant.price);
	const lineOverPrice = plant.compareAtPrice ? formatPrice(plant.compareAtPrice) : null;
    ...
    return (
        <Card className="group w-full max-w-sm overflow-hidden transition-all hover:shadow-lg flex flex-col">
            <CardHeader className="p-0">
                ...
                <Link href={productUrl} className="block overflow-hidden bg-muted relative aspect-square w-full">
                    ...
                    {discountPercent && (
                        <Badge className="absolute right-3 top-3 z-10" variant="destructive">
                            {discountPercent}% OFF
                        </Badge>
                    )}
                </Link>
            ...
            <CardContent className="p-4 flex-grow">
                ...
                <CardTitle className="text-l line-clamp-1">
                    <Link href={productUrl} className="hover:text-primary hover:underline transition-colors">
                        {plant.name}
                    </Link>
                </CardTitle>
