			...
			<div className="flex justify-between items-end mb-12">
				<div>
					...
					<p className="text-gray-500 font-bold uppercase tracking-widest text-xs mt-2">
						The latest from our greenhouse
					</p>
				</div>
				<Link
					href={ROUTES.CATEGORIES.ROOT}
					className="font-black uppercase tracking-tighter border-b-2 border-black pb-1 hover:text-green-700 hover:border-green-700 transition-all"
				>
					View All
				</Link>
			</div>
			...
			<div className="flex overflow-x-auto pb-10 gap-6 snap-x hide-scrollbar md:grid md:grid-cols-4 md:overflow-visible">
				{products.map((product) => {
					...
					return (
						<div
							key={product.id}
							className="shrink-0 w-[300px] snap-start md:w-full"
						>
