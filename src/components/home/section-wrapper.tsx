...
export function SectionWrapper({
	...
	let subtitleVisClass =
		"items-center gap-2 bg-emerald-50/50 border border-emerald-100 w-fit px-3 py-1.5 rounded-full";
				...
				>
					{hasMetaData
						? metaData.map((item) => {
								...
								let rowVisClass =
									"flex flex-col items-start md:flex-row md:items-center gap-0.5 md:gap-2";
								...
								return (
									<div key={item.label} className={rowVisClass}>
										...
										<span
											className={`text-[10px] md:text-xs font-bold uppercase tracking-widest flex items-center ${item.effect === "emerald-text" || item.effect === "emerald-badge" ? "text-emerald-600" : "text-gray-600"}`}
										>
