...
export function CartDrawer() { // Kept the name same so your Header doesn't break imports
    const items = useCartStore((state) => state.items);
    const cartCount = items.reduce((total, item) => total + item.quantity, 0);

    return (
        <Link href={ROUTES.CART || "/cart"} className="relative flex items-center p-2 text-muted-foreground hover:text-foreground transition-colors group">
