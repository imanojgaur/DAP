...
export function SearchBar() {
    ...
    const handleNavigate = (slug: string, type: "product" | "category") => {
        setIsOpen(false);
        setQuery("");
        const path = type === "category" ? `/collections/${slug}` : `/products/${slug}`;
        router.push(path);
    };
    ...
    return (
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
                <button type="button" className="flex items-center gap-4 text-sm text-muted-foreground hover:text-foreground bg-slate-100 w-full h-full ">
                    ...
                    <CommandList className="mt-6 max-h-[60vh] overflow-y-auto no-scrollbar">
                        {isLoading && (
                            <div className="flex items-center justify-center py-10">
                                ...
                                <span className="ml-3 text-lg text-muted-foreground">Syncing catalog...</span>
                            </div>
                        )}

                        {!isLoading && query.length > 0 && filteredPlants.length === 0 && filteredCats.length === 0 && (
                        ...
                        {!isLoading && query.length === 0 && (
                            <CommandGroup heading="Quick Links" className="text-muted-foreground">
                                <CommandItem onSelect={() => handleNavigate("indoor-plants", "category")} className="py-4 text-lg cursor-pointer">
                                    Indoor Plants
                                </CommandItem>
                                <CommandItem onSelect={() => handleNavigate("vastu-plants", "category")} className="py-4 text-lg cursor-pointer">
                                    Vastu Plants
                                </CommandItem>
                                <CommandItem onSelect={() => handleNavigate("best-sellers", "category")} className="py-4 text-lg cursor-pointer">
                                    Best Sellers
                                </CommandItem>
                            </CommandGroup>
                        )}
                        ...
                        {!isLoading && filteredCats.length > 0 && query.length > 0 && (
                            <CommandGroup heading="Collections" className="text-muted-foreground">
                        ...
                        {!isLoading && filteredPlants.length > 0 && query.length > 0 && (
                            <CommandGroup heading="Products" className="text-muted-foreground mt-4">
                                {filteredPlants.map((plant) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-lg font-medium text-foreground">{plant.name}</span>
                                            <span className="text-sm font-bold text-primary">₹{plant.price / 100}</span>
                                        </div>
                                    </CommandItem>
                                ))}
                            </CommandGroup>
